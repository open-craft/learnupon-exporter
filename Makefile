...
# PROJECT_ROOT is usually '/edx/src/learnupon-exporter/' on an Open edX devstack
...
quality: ## check coding style with pycodestyle and pylint
	pylint learnupon_exporter $(PROJECT_ROOT)setup.py
	pylint --py3k learnupon_exporter $(PROJECT_ROOT)setup.py
	pycodestyle $(PROJECT_ROOT)learnupon_exporter $(PROJECT_ROOT)setup.py
	isort --check-only --diff --recursive $(PROJECT_ROOT)learnupon_exporter $(PROJECT_ROOT)setup.py
...
test: clean
	python -m pytest --ds=cms.envs.test --no-cov --nomigrations $(PROJECT_ROOT)learnupon_exporter/tests/
...
extract_translations: ## extract strings to be translated, outputting .mo files
	rm -rf docs/_build
	cd learnupon-exporter && ../manage.py makemessages -l en -v1 -d django
	cd learnupon-exporter && ../manage.py makemessages -l en -v1 -d djangojs

compile_translations: ## compile translation files, outputting .po files for each supported language
	cd learnupon-exporter && ../manage.py compilemessages

detect_changed_source_translations:
	cd learnupon-exporter && i18n_tool changed
...
dummy_translations: ## generate dummy translation (.po) files
	cd learnupon_exporter && i18n_tool dummy
